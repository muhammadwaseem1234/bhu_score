...
import { LandParcelProfile, WatchlistSubscription, ParcelAlertEvent } from '@/src/types';
import { api } from '@/src/services/api';
import { INITIAL_PARCELS } from '@/src/data/mockParcels';
import { Navbar } from '@/src/components/Navbar';
import { ScoreGauge } from '@/src/components/ScoreGauge';
import { CadastralMapView } from '@/src/components/CadastralMapView';
import { OwnershipTimeline } from '@/src/components/OwnershipTimeline';
import { CersaiEncumbrances } from '@/src/components/CersaiEncumbrances';
import { LitigationDossier } from '@/src/components/LitigationDossier';
import { PublicNoticesView } from '@/src/components/PublicNoticesView';
import { WatchlistManager } from '@/src/components/WatchlistManager';
import { AiLegalOpinionModal } from '@/src/components/AiLegalOpinionModal';
import { BureauReportPrintView } from '@/src/components/BureauReportPrintView';
import { LenderPortfolioView } from '@/src/components/LenderPortfolioView';
import { EntityMatchingVisualizer } from '@/src/components/EntityMatchingVisualizer';
import { LandDetailsView } from '@/src/components/LandDetailsView';
import {
  MapPin,
  FileText,
  Sparkles,
...
export default function HomePage() {
  ...
  const [activeSubSection, setActiveSubSection] = useState<'overview' | 'land-details' | 'cadastral' | 'lineage' | 'encumbrance' | 'litigation' | 'notices'>('overview');
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 font-sans selection:bg-emerald-600 selection:text-white pb-16">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 pt-6 space-y-6">
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none text-xs">
              ...
              {parcels.map((p) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl border font-mono transition-all whitespace-nowrap flex items-center gap-2 cursor-pointer ${
                ...
                >
                  <span>
                    {p.stateSurveyNo} ({p.state.split(' ')[0]})
                  </span>
                  <span
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap items-center gap-y-1 gap-x-4 text-xs text-slate-600">
                    <span className="flex items-center gap-1 text-slate-700">
                      ...
                      {currentParcel.villageName}, {currentParcel.talukTehsil}, {currentParcel.district},{' '}
                      {currentParcel.state} - {currentParcel.pincode}
                    </span>
                    <span>•</span>
                    <span>
                      Extent: <strong className="text-slate-900">{currentParcel.boundary.totalAreaAcres} Acres</strong>{' '}
                      ({currentParcel.boundary.totalAreaSqFt.toLocaleString('en-IN')} sq.ft)
                    </span>
                    <span>•</span>
                    <span>
                      Zoning: <strong className="text-emerald-700">{currentParcel.zoning}</strong>
                    </span>
                  </div>
                </div>
            ...
            <div className="flex items-center gap-2 border-b border-slate-200 pb-2 overflow-x-auto scrollbar-none text-xs font-semibold">
              <button
                ...
                className={`px-3.5 py-2 rounded-xl transition-colors flex items-center gap-1.5 whitespace-nowrap cursor-pointer ${
              ...
              <button
                onClick={() => setActiveSubSection('land-details')}
                className={`px-3.5 py-2 rounded-xl transition-colors flex items-center gap-1.5 whitespace-nowrap cursor-pointer ${
                  activeSubSection === 'land-details'
                    ? 'bg-emerald-100 text-emerald-900 border border-emerald-300 font-bold'
                    : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/60'
                }`}
              >
                <FileText className="w-3.5 h-3.5 text-emerald-700" />
                <span>Land Details (RTC / Pahani)</span>
              </button>
        ...
        {activeTab === 'portfolio' && (
          <div>
            <LenderPortfolioView parcels={parcels} onSelectParcel={handleSelectParcel} />
          </div>
        )}
      ...
      {isPrintModalOpen && (
        <BureauReportPrintView parcel={currentParcel} onClose={() => setIsPrintModalOpen(false)} />
      )}
    </div>
  );
}
